...
import { generateGuide } from '../utils/pdf';
...
export const leadsApi = {
  ...
  downloadGuide: async (email: string): Promise<Blob> => {
    try {
      logInfo('Generating guide PDF', { email });
      
      const doc = generateGuide();
      const pdfBlob = doc.output('blob');
      
      logInfo('Guide PDF generated successfully', { email });
      
      return pdfBlob;
    } catch (error) {
      logError(error as Error, { context: 'Guide Generation' });
      throw new Error('Error al generar la guía. Por favor, inténtalo de nuevo.');
    }
  }
};